<template>
    <div>
        ...
        <div id="winner-show" class="py-5" style="display: none">
            ...
            -->
            <p v-if="winner_code" class="digit-box display-3 text-center m-0 d-none">
                {{ winner_code }}
            </p>
    ...
    export default {
        ...
        data() {
            return {
                ...
                winner_code: 0,
                winner_mobile: '**********',
                status: 'در حال جستجو برندگان <i class="mdi mdi-18px mdi-loading mdi-spin top-2 mr-2"></i>',
                winners: [],
        ...
        methods: {
            start_draw() {
                ...
                if (btn.hasClass('loading')) {
                    ...
                    if (parseInt(vm.first_code) != 0 && parseInt(this.first_code) <= this.codes) {
            ...
            check_winners(code_index) {
                var vm = this;
                vm.status = 'در حال جستجو برندگان <i class="mdi mdi-18px mdi-loading mdi-spin top-2 mr-2"></i>';
                ...
                if (false/*vm.winner_code != vm.winner_codes[code_index]*/) {
                    ...
                    axios.post($('meta[name="page-url"]').attr('content'), {
                        code: vm.winner_codes,
                    }).then(function (response) {
                        if (response && response.status == 200) {
                            if (response.data.status == "success") {
                                vm.serials = JSON.parse(response.data.serials);
                                vm.show_serials(0, vm.serials[0]);
                            }
            ...
            show_serials(index, currentSerial) {
                var vm = this;
                vm.winner_index = ++index;
                vm.winner_mobile = '**********';
                vm.status = 'تبریک به شما :)';

                var arrSerial = Array.from(currentSerial);
                arrSerial[4] = '*';
                arrSerial[5] = '*';
                arrSerial[6] = '*';
                this.winners.push(arrSerial.join(""));

                vm.winner_mobile = arrSerial.join("");

                if (index < this.serials.length) {
                    setTimeout(function () {
                        vm.show_serials(index, vm.serials[index]);
                        console.log(index);
                    }, 100);
                } else {
                    vm.status = 'قرعه کشی تمام شد! تبریک به برندگان';
